...
public interface IBillRepository extends JpaRepository<Bill, String> {
	 ...
	 * @deprecated use {@link #getBillsForStaffRanged(Date, Date)}
	 */
	@Deprecated
	 ...
	 * @param from
	 * @param to
	 * @return all bills opened &ge; from and &le; to
	 */
	@Query("from Bill where billOpened >= ?1 AND billOpened <= ?2 AND internalConsumer is NOT NULL")
	public Collection<Bill> getBillsForStaffRanged(Date from, Date to);
