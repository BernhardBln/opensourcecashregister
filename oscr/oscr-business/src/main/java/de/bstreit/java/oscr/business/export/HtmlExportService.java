...
@Named
public class HtmlExportService implements IExportService, IBillChangedListener, Runnable {

  private static final Logger logger = LoggerFactory.getLogger(HtmlExportService.class);
  ...
  public void runInBackground() {
    if (stopService) {
      throw new IllegalStateException("Service did run already, do not re-use!");
    }
  ...
  private void enrichAndExportFile(String htmlFile) {
    ...
    final String changedHtmlFile = htmlFile.replace("$CONTENT", sb.toString());

    FileOutputStream fileOutputStream = null;

    try {
      fileOutputStream = new FileOutputStream(exportDir + "/export.htm");
      IOUtils.write(changedHtmlFile, fileOutputStream, Charset.forName("UTF-8"));
