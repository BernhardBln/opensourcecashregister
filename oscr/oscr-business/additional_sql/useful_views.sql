CREATE OR REPLACE VIEW BILLS_WITH_PRODUCTS AS 
	SELECT 
		S.TYPE, 
		S.NAME, 
		PRICEVALUE, 
	...
	WHERE 
		...
		OFFEREDITEM_ID = S.ID
		AND (b.INTERNALCONSUMER_ID IS NULL OR EXISTS (SELECT * FROM USER U WHERE U.ID = INTERNALCONSUMER_ID AND U.customer = true));
